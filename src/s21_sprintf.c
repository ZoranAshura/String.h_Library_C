  ...
  switch (sign) {
    ...
    case 'c':
      ...
      if (parameters->length == LENGTH_SHORT)
        *sptr = add_char(*sptr, (short)va_arg(ap, int), parameters);
      else if (parameters->length == LENGTH_LONG)
        *sptr = add_char(*sptr, va_arg(ap, long), parameters);
      else
...
char* add_int(char* string, long long number, int format,
  ...
  if ((parameters->sharp && number != 0) || parameters->isPtr) {
...
char* add_str(char* result, const char* string, sprintf_struct* parameters) {
  ...
  s21_size_t size_string = s21_strlen(string);
  if ((s21_strncmp(string, "nan", size_string) == 0) ||
      (s21_strncmp(string, "-inf", size_string) == 0) ||
      (s21_strncmp(string, "inf", size_string) == 0)) {
    parameters->space_symbol = ' ';
  }
